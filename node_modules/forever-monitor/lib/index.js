...
class Monitor extends EventEmitter {
  constructor (script, options = {}) {
    ...
    plugins.logger.attach.call(this, this.options);
    if (options.watch) {
  ...
  async start(restart) {
    ...
    if (this.running && !restart) {
      process.nextTick(() => {
        this.emit('error', new Error('Cannot start process that is already running.'));
      });
      return this;
    }
    ...
    if (!child) {
      process.nextTick(() => {
        this.emit('error', new Error('Target script does not exist: ' + this.args[0]));
      });
      return this;
    }
    ...
    process.nextTick(() => {
      this.emit(restart ? 'restart' : 'start', this.childStatus);
    });
    ...
    child.once('exit', (code, signal) => {
      ...
      if (this.options.minUptime) {
        upTime = this.options.minUptime - (Date.now() - this.startTime);
      }
      ...
      if (this.startAfterKill) {
        this.startCount = 0;
        this.startAfterKill = false;
      }
      
      this.emit('child:exit', code, signal, this.childStatus);

      if (this.startCount >= this.options.maxTry) {
        letChildDie();
      } else {
        if (upTime > 0) {
          setTimeout(restartChild, upTime);
        } else {
          restartChild();
        }
      }
  ...
  //
  restart() {
    this.startCount = this.startCount || 0;
    return !this.running
      ? this.start(true)
      : this._killChild(true);
  }
  ...
  _killChild(startAfterKill) {
    ...
    if (!child || !this.running) {
      ...
      this.startAfterKill = startAfterKill;
      // startAfterKill will take effect in child.on('exit', callback) of function start;
  ...
  send(msg) {
    ...
    if (!child || !this.running) {
      process.nextTick(() => {
        this.emit('error', new Error('Cannot send to process that is not running.'));
      });
    }
