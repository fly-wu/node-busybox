...
var readDir = require('fs-readdir-recursive/with-dir')
...
module.exports = function staticCache(dir, options, files) {
  ...
  options.prefix = options.prefix ? options.prefix.replace(/\/*$/, '') : '/'
  ...
  // options.dirContent = options.dirContent ? options.dirContent : (fullpath => 'dir')
  ...
  return async (ctx, next) => {
    ...
    if (!file) {
      ...
      if (!s.isFile() && !s.isDirectory()) return await next()
      if (s.isDirectory() && !options.dirContent) return await next()

      file = loadFile(filename, dir, options, files)
    }

    // path to dir should ends with '/'
    if (file.isDirectory && !ctx.path.endsWith('/')) {
      ctx.status = 307;
    ...
    if (!file.buffer) {
      ...
      if (stats.isFile()) {
        ...
        file.length = options.dirContent(file).length
      }
    }
    ...
    if (file.isDirectory) {
      stream = new require('stream').Readable({
        read() {
          this.push(options.dirContent(file))
          this.push(null)
        }
      })
    } else {
...
function loadFile(name, dir, options, files) {
  ...
  if (stats.isFile()) {
    ...
    if (!options.dirContent) {
      return;
    }
    obj.type = 'text/html'
    buffer = Buffer.from(options.dirContent(obj))
    obj.length = buffer.length
    obj.isDirectory = true
    if (pathname.length > 0 && pathname.endsWith('/')) {
      files.set(`${pathname.slice(0, -1)}`, obj)
    } else {
